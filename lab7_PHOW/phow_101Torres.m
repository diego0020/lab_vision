function phow_101Torres(Claszs,numIm,spatial,Folder)
...
conf.numClasses = 102 ;
conf.numWords = 600 ;
conf.numSpatialX = spatial ;
conf.numSpatialY = spatial ;
conf.quantizer = 'kdtree' ;
conf.svm.C = 10 ;

conf.svm.solver = 'sdca' ;
nel = numel(dir(Folder))-2;
testNom = strcat(Folder,'_',num2str(nel));
Attemp = fullfile(Folder,testNom);
mkdir(conf.dataDir,Attemp)
%conf.svm.solver = 'sgd' ;
...
conf.vocabPath = fullfile(conf.dataDir,Attemp, [conf.prefix '-vocab.mat']) ;
conf.histPath = fullfile(conf.dataDir,Attemp, [conf.prefix '-hists.mat']) ;
conf.modelPath = fullfile(conf.dataDir,Attemp, [conf.prefix '-model.mat']) ;
conf.resultPath = fullfile(conf.dataDir,Attemp, [conf.prefix '-result']) ;
...
savefig(fullfile(conf.dataDir,Attemp, strcat(testNom,'.fig')))
print(fullfile(conf.dataDir,Attemp, strcat(testNom)),'-dpng')
